...
def home():
    ...
    return render_template('register.html',names=names,rolls=rolls,times=times,dates=dates,l=l,totalreg=totalreg(),datetoday2=datetoday2()) 
...
def start():
    ...
    if 'face_recognition_model.pkl' not in os.listdir('static/'):
        return render_template('register.html',totalreg=totalreg(), datetoday2=datetoday2(), mess='There is no trained model in the trainer folder. Please add a new face to continue.')
...
def add():
    ...
    while 1:
        ...
        for (x,y,w,h) in faces:
            cv2.rectangle(frame, (x, y), (x+w, y+h), (255, 0, 20), 2)
            cv2.putText(frame, f'Images Captured: {i}/50', (30,30), font, 1, (255, 0, 20), 2, cv2.LINE_AA)
            if j%10 ==0:
...
def get_attendence():
    names,rolls,times,dates,l = extract_attendance()
    return render_template('attendence.html',names=names, rolls=rolls, times=times, dates = dates, l=l, totalreg=totalreg(), datetoday2=datetoday2())
